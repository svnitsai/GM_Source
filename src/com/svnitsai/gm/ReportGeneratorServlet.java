...
public class ReportGeneratorServlet extends HttpServlet
{
	public static enum ReportType
	{
		...
		PayableMorning ("Daily Payables", "SP_M_Final.jasper", "DailyPayables.pdf"),
		SupplierEvening ("Supplier Payments", "SP_Evening_Final.jasper", "DailySupplierPayments.pdf"),
		CollectionsMorning("Invoice Report", "Daily_Collection_Morning.jasper", "DailyInvoices.pdf"),
		CollectionsEvening("Collections Report", "Daily_Collection_Evening.jasper", "DailyCollections.pdf"),
		LedgerReport("Ledger Report", "Payment_Collection_Main.jasper", "LedgerReport.pdf");
			...
			throws ServletException, IOException
	{
		...
		if(reportTypeEnum != null)
		{
			...
			String jasperName = basePath + "jrxml\\" + reportTypeEnum.getJasperFileName();
	        ...
	        RequestDispatcher dispatcher = request.getRequestDispatcher("/" + reportTypeEnum.getPdfFileName());
			dispatcher.forward(request, response);
		}
		else
		{
			RequestDispatcher dispatcher = request.getRequestDispatcher("/web/reports.jsp?status=failed");
			dispatcher.forward(request, response);
		}
   ...
   public static void generateReport(String jasperName, String pdfName, Map parameters) 
   {
	   ...
	   try 
	   {
          ...
          String printFileName = JasperFillManager.fillReportToFile(jasperName,
        		  													parameters, conn);
          if (printFileName != null) 
          {
             ...
             * 1- export to PDF
             */
            JasperExportManager.exportReportToPdfFile(printFileName, pdfName);
          }
	   } 
	   catch (JRException e) 
	   {
